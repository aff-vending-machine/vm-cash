...
def coin_collecting(timeout_queue, msg_queue, serT, cond):
    ...
    while True:
        ...
        if not timeout_queue.empty():
            ...
            print("coin_collecting got event: %s, | %s | %s" % (tname, coin_timeout, datetime.datetime.now().isoformat()))
            timeout_queue.task_done()
        
        if not msg_queue.empty():
            thread_status = msg_queue.get()
            print("Thread_status got event: %s, %s, | %s" % (tname, thread_status, datetime.datetime.now().isoformat()))
            msg_queue.task_done()
        ...
        if coin_timeout > 0:
            ...
            if thread_status == 'start':
                ...
                try:
                    ...
                    print("Reader: %s | Coin_Value: %s | Coin_Timeout: %s" % (repr(reader), coin_value, coin_timeout))
                    ...
                    print("Exception in mei recv thread: ",str(e))
            ...
            endt = time.time()
            print("Coin Value: %r | DiffTime: %r" % (recv_value, endt - startt))
...
class CoinResource:
    ...
    def on_post(self, req, resp):
        ...
        #### Action 1 ####
        if coin_cmd == 'enable' or coin_cmd == 'disable':
            if mei_sts[0]:
                if mei_sts[1]:
        ...
        #### Action 2 ####
        elif coin_cmd == 'status' or coin_cmd == 'info':
            if mei_sts[0]:
                resp_data = {
                    'success': True,
                    'action': 2,
